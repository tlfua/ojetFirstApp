...
export class OjDataGrid extends OjDataGridBase {
   ...
   */
  async getCellContents(itemLocator: { key: cellSubId }): Promise<WebElement | null> {
  ...
  async findCell(itemLocator: cellSubId): Promise<SlotProxy> {
  ...
  async findHeader(itemLocator: headerSubId): Promise<SlotProxy> {
    await this.whenReady();
  ...
  async findLabel(itemLocator: labelSubId): Promise<SlotProxy> {
    await this.whenReady();
    let axis = itemLocator.axis;
    let level = itemLocator.level != null ? itemLocator.level : 0;
